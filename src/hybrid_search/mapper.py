...
class MappingDictCreator:
    ...
    def _clean_data(self):
        ...
        self.df_data.dropna(subset=["C"], inplace=True)  # Drop rows with NaN in column 'C'

    def create_mapping_dict(self, source_columns: list, target_column: str):
        mapping_dict = {}
        for col in source_columns:
            col_dict = pd.Series(self.df_data[target_column].values, index=self.df_data[col]).to_dict()
