...
class HybridSearch:
    ...
    def bm25_search(self, query: str, top_n: int = 10):
        ...
        ranked_results = [
            (
                top_n_candidates[i],
                top_n_bm25_scores[i],
                self.base_mapping[top_n_candidates[i]],
            )
            for i in range(len(top_n_candidates))
        ]
        return ranked_results

    def transformer_search(self, query: str, top_n: int = 10):
        # Calculate Sentence Transformers Similarity using FAISS
        query_embedding = self.model.encode(query, convert_to_tensor=False).reshape(
            1, -1
        )
        ...
        faiss_scores = [
            util.pytorch_cos_sim(
                query_embedding, np.expand_dims(self.corpus_embeddings[i], axis=0)
            )
            .numpy()
            .flatten()[0]
    ...
    def hybrid_search(
        ...
        bm25_top_n_scores = [
            bm25_scores[self.corpus.index(candidate)] for candidate in top_n_candidates
        ]

        # Normalize BM25 scores and transformer scores
        if np.max(bm25_top_n_scores) != np.min(bm25_top_n_scores):
            bm25_top_n_scores = (bm25_top_n_scores - np.min(bm25_top_n_scores)) / (
                np.max(bm25_top_n_scores) - np.min(bm25_top_n_scores)
            )
        else:
            bm25_top_n_scores = np.array(bm25_top_n_scores)

        if np.max(transformer_scores) != np.min(transformer_scores):
            transformer_scores = (transformer_scores - np.min(transformer_scores)) / (
                np.max(transformer_scores) - np.min(transformer_scores)
            )
        else:
        ...
        # Refined Scoring
        agreement_factor = np.multiply(
            bm25_top_n_scores, transformer_scores
        )  # Emphasize agreement
        combined_scores = (
            bm25_weight * bm25_top_n_scores
            + transformer_weight * transformer_scores
            + agreement_factor
        )
