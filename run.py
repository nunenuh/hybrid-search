...
def initialize(file, progress=gr.Progress(track_tqdm=True)):
    ...
    try:
        ...
        return (
            gr.update(value="File uploaded and engine initialized successfully.", visible=True),
    ...
    with gr.Blocks() as iface:
        with gr.Column() as col:
            ...
            with gr.Column(visible=True) as upload_row:
                upload = gr.File(label="Upload Excel File", file_types=[".xlsx"], scale=1)
                init_status = gr.Textbox(label="Initialization Status", interactive=False, visible=True, scale=1)
            ...
            upload_button.click(
                initialize,
                inputs=upload,
                outputs=[init_status, upload_row, search_col]
            )
